...
export default function ApartmentsPage() {
  ...
  const handlePhotoUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (file.size > 5 * 1024 * 1024)
      return toast.error("Dosya çok büyük (Max 5MB)");
    ...
    try {
      ...
      setApartments((prev) =>
        prev.map((a) =>
          a.id === editing.id ? { ...a, owner_photo: newPhotoUrl } : a,
        ),
      );
    ...
    <div className="p-5 md:p-8 space-y-6">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-slate-500 text-sm mt-1">
            Cumhuriyet Apartmanı'nın 18 dairesi
          </p>
        </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
        {filtered.map((apt) => (
          <div
            key={apt.id}
            className="relative group bg-white rounded-xl border border-slate-200 p-5 shadow-sm hover:shadow-xl hover:scale-105 transition-all duration-300 transform"
          >
            <div className="flex items-start justify-between mb-3 relative z-10">
                ...
                <div>
                  ...
                  <div className="flex items-center gap-2">
                    <p className="text-xs text-slate-500">
                      Kat {apt.floor} · Daire {apt.number}
                    </p>
                    <span
                      className={`px-2 py-0.5 rounded-full text-[10px] font-bold tracking-tight shadow-sm ${apt.room_type === "3+1" ? "bg-blue-100 text-blue-700" : "bg-orange-100 text-orange-700"}`}
                    >
                      {apt.room_type}
                    </span>
                  </div>
              ...
              <button
                ...
                className="p-1.5 z-20 relative text-slate-400 hover:text-primary hover:bg-primary/10 rounded-lg transition-colors"
              >
            ...
            <div
              className="absolute left-1/2 -ml-32 bottom-full mb-4 w-64 rounded-2xl p-[2px] opacity-0 pointer-events-none scale-95 origin-bottom group-hover:opacity-100 group-hover:scale-100 transition-all duration-300 z-50 shadow-2xl overflow-hidden
            ...
            >
              <div className="w-full h-full bg-white rounded-[14px] p-4 relative z-10 flex flex-col items-center text-center">
                <div className="relative w-16 h-16 rounded-full overflow-hidden border-2 border-slate-100 mb-3 bg-slate-100 flex-shrink-0">
                  {apt.owner_photo ? (
                    <img
                      src={`/api${apt.owner_photo}`}
                      alt={apt.owner_name}
                      className="w-full h-full object-cover"
                    />
                  ) : (
                    <span className="material-symbols-outlined text-slate-400 text-3xl absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2">
                      person
                    </span>
                  )}
                </div>
                <h4 className="font-bold text-base leading-tight text-slate-900 mb-1">
                  {apt.owner_name}
                </h4>
                <p className="text-xs text-primary font-medium mb-3">
                  {apt.profession || "Daire Sakini"}
                </p>
                {apt.notes && (
      ...
      {editing && (
        ...
        >
          ...
          >
            ...
            <div>
              ...
              <div className="flex items-center gap-4 mb-3">
                <div className="w-12 h-12 rounded-full overflow-hidden bg-slate-100 flex-shrink-0 relative border border-slate-200">
                  {editing.owner_photo ? (
                    <img
                      src={`/api${editing.owner_photo}`}
                      alt="Owner"
                      className="w-full h-full object-cover"
                    />
                  ) : (
                    <span className="material-symbols-outlined text-slate-400 absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2">
                      person
                    </span>
                  )}
                </div>
              ...
              <input
                ...
                onChange={(e) =>
                  setEditing({ ...editing, owner_name: e.target.value })
                }
              />
            </div>
            <div className="grid grid-cols-2 gap-3">
              ...
              <div>
                ...
                <select
                  ...
                  onChange={(e) =>
                    setEditing({ ...editing, room_type: e.target.value })
                  }
                >
            ...
            <button
              onClick={handleSave}
              className="w-full bg-primary text-white py-2.5 rounded-lg font-bold hover:opacity-90"
            >
              Kaydet
            </button>
          </div>
      ...
      <div className="bg-primary/5 rounded-xl p-5 flex items-center gap-4">
        ...
        <p className="text-sm text-slate-600">
          <strong className="text-primary">KALI YAPI / KALİ YAPI</strong>{" "}
          dairelerinin sakinleri henüz belirlenmemiştir. Güncellemek için
          düzenleme butonunu kullanın.
        </p>
      </div>
    </div>
  );
