...
interface Apartment { id: number; number: number; owner_name: string; floor: number; notes: string; }
  ...
  return (
    <div className="p-5 md:p-8 space-y-6">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
        {filtered.map(apt => (
          <div key={apt.id} className="bg-white dark:bg-slate-900 rounded-xl border border-slate-200 dark:border-slate-800 p-5 hover:shadow-md transition-shadow">
            ...
            {apt.notes && <p className="text-xs text-slate-500 bg-slate-50 dark:bg-slate-800 rounded-lg px-3 py-2 mt-2">{apt.notes}</p>}
          </div>
        ))}
      </div>
