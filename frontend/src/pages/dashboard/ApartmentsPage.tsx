        ...
        {filtered.map(apt => (
          <div key={apt.id} className="relative group bg-white rounded-xl border border-slate-200 p-5 shadow-sm hover:shadow-xl hover:scale-105 transition-all duration-300 transform">
            <div className="flex items-start justify-between mb-3 relative z-10">
              ...
              <button onClick={() => setEditing({ ...apt })} className="p-1.5 z-20 relative text-slate-400 hover:text-primary hover:bg-primary/10 rounded-lg transition-colors">
            ...
            <div className="absolute left-1/2 -ml-32 bottom-full mb-4 w-64 rounded-2xl p-[2px] opacity-0 pointer-events-none scale-95 origin-bottom group-hover:opacity-100 group-hover:scale-100 transition-all duration-300 z-50 shadow-2xl overflow-hidden
              ...
              <div className="w-full h-full bg-white rounded-[14px] p-4 relative z-10 flex flex-col items-center text-center">
                ...
                {apt.notes && <div className="text-xs text-slate-500 bg-slate-50 rounded-lg p-2 w-full text-center">{apt.notes}</div>}
              </div>
        ...
        <div className="fixed inset-0 bg-black/60 z-50 flex items-center justify-center p-4" onClick={() => setEditing(null)}>
          <div className="bg-white rounded-2xl p-6 max-w-md w-full space-y-4" onClick={e => e.stopPropagation()}>
            ...
            <div>
              ...
              <div className="flex items-center gap-4 mb-3">
                ...
                <div>
                  <label className="text-xs font-bold text-primary cursor-pointer hover:underline flex items-center gap-1">
                    <span className="material-symbols-outlined text-[16px]">upload</span> Fotoğraf Yükle
              ...
              <input className="w-full px-3 py-2.5 bg-slate-50 border border-slate-200 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-primary/30" value={editing.owner_name} onChange={e => setEditing({ ...editing, owner_name: e.target.value })} />
            </div>
            <div className="grid grid-cols-2 gap-3">
              <div>
                <label className="block text-sm font-medium mb-1.5">Kat</label>
                <input type="number" className="w-full px-3 py-2.5 bg-slate-50 border border-slate-200 rounded-lg text-sm" value={editing.floor} onChange={e => setEditing({ ...editing, floor: +e.target.value })} />
              </div>
              <div>
                <label className="block text-sm font-medium mb-1.5">Daire Tipi</label>
                <select className="w-full px-3 py-2.5 bg-slate-50 border border-slate-200 rounded-lg text-sm" value={editing.room_type} onChange={e => setEditing({ ...editing, room_type: e.target.value })}>
            ...
            <div>
              ...
              <input type="text" className="w-full px-3 py-2.5 bg-slate-50 border border-slate-200 rounded-lg text-sm" value={editing.profession || ''} onChange={e => setEditing({ ...editing, profession: e.target.value })} />
            </div>
