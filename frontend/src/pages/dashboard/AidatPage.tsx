      ...
      {addingPeriod && (
        <div className="bg-white border border-slate-200 rounded-xl p-5">
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            ...
            <div>
              <label className="block text-xs font-medium text-slate-500 mb-1">2+1 Aidat (₺)</label>
              <input type="number" className="w-full px-3 py-2 bg-slate-50 border border-slate-200 rounded-lg text-sm border-orange-200" value={newPeriod.amount_2plus1} onChange={e => setNewPeriod(p => ({ ...p, amount_2plus1: +e.target.value }))} />
            </div>
            <div>
              <label className="block text-xs font-medium text-slate-500 mb-1">3+1 Aidat (₺)</label>
              <input type="number" className="w-full px-3 py-2 bg-slate-50 border border-slate-200 rounded-lg text-sm border-blue-200" value={newPeriod.amount_3plus1} onChange={e => setNewPeriod(p => ({ ...p, amount_3plus1: +e.target.value }))} />
            </div>
          </div>
                  ...
                  return (
                    <tr key={p.id} className={`hover:bg-slate-50 transition-colors ${rowBorderCls}`}>
                      ...
                      <td className="px-5 py-3.5 text-sm">
                        <span className={`px-2 py-0.5 rounded-full text-[10px] font-bold ${p.room_type === '3+1' ? 'bg-blue-100 text-blue-700' : 'bg-orange-100 text-orange-700'}`}>
                          {p.room_type}
                        </span>
                      </td>
