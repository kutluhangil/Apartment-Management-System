...
export default function AidatPage() {
  ...
  const exportPDF = () => {
    try {
      ...
      doc.setFontSize(12);
      doc.text(
        `Donem: ${MONTHS[selectedAidat.month - 1]} ${selectedAidat.year}`,
        14,
        30,
      );
  ...
  const exportExcel = () => {
    try {
      ...
      toast.error("Excel oluşturulurken hata!");
    }
  };

  const handleStatusChange = async (paymentId: number, status: string) => {
    try {
      await aidatsApi.updatePayment(paymentId, {
        status,
  ...
  const handleDeletePeriod = async () => {
    ...
    if (
      !window.confirm(
        `"${label}" dönemini silmek istediğinize emin misiniz?\nTüm ödeme kayıtları da silinecektir.`,
      )
    )
      return;
  ...
  const handleCreatePeriod = async () => {
    try {
      if (
        !newPeriod.amount_2plus1 ||
        newPeriod.amount_2plus1 <= 0 ||
        !newPeriod.amount_3plus1 ||
        newPeriod.amount_3plus1 <= 0
      ) {
        toast.error("Lütfen geçerli aidat tutarları girin.");
        return;
      }
      await aidatsApi.create({
        month: newPeriod.month,
        year: newPeriod.year,
        amount_2plus1: newPeriod.amount_2plus1,
        amount_3plus1: newPeriod.amount_3plus1,
      });
  ...
  const handleExpenseSubmit = async (e: React.FormEvent) => {
    ...
    if (!expenseForm.title || !expenseForm.amount || !expenseForm.date) {
      toast.error("Başlık, tutar ve tarih zorunludur.");
      return;
    }
    setLoading(true);
    ...
    <div className="p-5 md:p-8 space-y-6">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
        <div>
          ...
          {selectedAidat && (
            <p className="text-slate-500">
              {MONTHS[selectedAidat.month - 1]} {selectedAidat.year} Dönemi
            </p>
          )}
        </div>
        <div className="flex gap-2 self-start">
          ...
          >
            <span className="material-symbols-outlined text-lg">add</span> Yeni
            Dönem
          </button>
        </div>
      </div>
        ...
        <div className="bg-white border border-slate-200 rounded-xl p-5">
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            ...
            <div>
              ...
              <input
                type="number"
                className="w-full px-3 py-2 bg-slate-50 border border-slate-200 rounded-lg text-sm border-orange-200"
                value={newPeriod.amount_2plus1}
                onChange={(e) =>
                  setNewPeriod((p) => ({
                    ...p,
                    amount_2plus1: +e.target.value,
                  }))
                }
              />
            </div>
            <div>
              ...
              <input
                type="number"
                className="w-full px-3 py-2 bg-slate-50 border border-slate-200 rounded-lg text-sm border-blue-200"
                value={newPeriod.amount_3plus1}
                onChange={(e) =>
                  setNewPeriod((p) => ({
                    ...p,
                    amount_3plus1: +e.target.value,
                  }))
                }
              />
            </div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        <div className="bg-white p-5 rounded-xl border border-slate-200">
          <p className="text-xs text-slate-500 mb-1">Beklenen Toplam</p>
        ...
        <div className="bg-white p-5 rounded-xl border border-slate-200">
          <p className="text-xs text-slate-500 mb-1">Ödenmemiş</p>
          <p className="text-xl font-bold text-red-500">{stats.unpaid_count}</p>
        </div>
        <div className="bg-white p-5 rounded-xl border border-slate-200">
          ...
          <div className="p-5 border-b border-slate-100 flex flex-wrap gap-4 items-center justify-between">
            ...
            <div className="flex flex-wrap items-center gap-4">
              ...
              <div className="flex gap-2">
                <button
                  onClick={exportPDF}
                  className="flex items-center gap-1 px-3 py-1 rounded-lg border border-slate-200 bg-white hover:bg-slate-50 text-xs font-medium text-slate-600 transition-colors"
                >
            ...
            <table className="w-full text-left">
              ...
              <tbody className="divide-y divide-slate-100">
                {payments.map((p) => {
                  ...
                  if (p.status === "paid")
                    rowBorderCls = "border-l-4 border-green-400";
                  else if (p.status === "unpaid")
                    rowBorderCls = "border-l-4 border-red-400";

                  return (
                    <tr
                      key={p.id}
                      className={`hover:bg-slate-50 transition-colors ${rowBorderCls}`}
                    >
                      ...
                      <td className="px-5 py-3.5 text-sm">
                        <span
                          className={`px-2 py-0.5 rounded-full text-[10px] font-bold ${p.room_type === "3+1" ? "bg-blue-100 text-blue-700" : "bg-orange-100 text-orange-700"}`}
                        >
                          {p.room_type}
                        </span>
                      </td>
          ...
          <div className="bg-white p-5 rounded-xl border border-slate-200">
            ...
            <form onSubmit={handleExpenseSubmit} className="space-y-3">
              <div>
                ...
                <input
                  className="w-full px-3 py-2 bg-slate-50 border border-slate-200 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-primary/30"
                ...
                >
                  ...
                  <p className="text-xs text-slate-500">
                    {file ? (
                      ...
                      <>
                        Sürükleyin veya{" "}
                        <span className="text-primary font-bold">seçin</span>
                      </>
                    )}
                  </p>
                  <input
              ...
              <button
                type="submit"
                disabled={loading}
                className="w-full bg-primary text-white py-2.5 rounded-lg text-sm font-bold flex items-center justify-center gap-2 hover:opacity-90 disabled:opacity-60"
              >
                {loading ? (
                  <>
                ...
                >
                  ...
                  <div className="text-right">
                    ...
                    {exp.invoice_path && (
                      <button
                        onClick={() =>
                          setPreviewUrl(`/uploads/${exp.invoice_path}`)
                        }
                        className="text-[10px] text-primary flex items-center gap-1 justify-end"
                      >
