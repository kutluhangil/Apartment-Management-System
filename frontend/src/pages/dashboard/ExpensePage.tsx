...
export default function ExpensePage() {
  ...
  return (
    <div className="space-y-6">
      <div>
        <h1 className="text-2xl font-black tracking-tight text-white">Gelir / Gider</h1>
        <p className="text-white/40 text-sm mt-1">Tüm finansal kayıtlar ve faturalar</p>
      </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
        <div className="bg-zinc-900 border border-white/[0.07] p-5 rounded-2xl flex items-center gap-4">
          ...
          <div>
            ...
            <p className="text-xl font-black text-emerald-400">{formatCurrency(summary.totalIncome)}</p>
          </div>
        </div>
        <div className="bg-zinc-900 border border-white/[0.07] p-5 rounded-2xl flex items-center gap-4">
          ...
          <div>
            ...
            <p className="text-xl font-black text-rose-400">{formatCurrency(summary.totalExpense)}</p>
          </div>
        </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-5">
        ...
        <div className="lg:col-span-8 space-y-4">
          ...
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
            ...
            <div className="flex items-center gap-2 flex-wrap">
              <div className="flex items-center gap-2 bg-white/5 border border-white/10 rounded-xl px-3 py-2 focus-within:border-indigo-500/30 transition-all">
                <span className="text-xs text-white/40 whitespace-nowrap">Tarih:</span>
                <input type="month" value={monthFilter} onChange={e => { setMonthFilter(e.target.value); setPage(1); }}
                  className="bg-transparent text-sm text-white focus:outline-none min-w-min" />
                {monthFilter && (
                  <button onClick={() => { setMonthFilter(''); setPage(1); }} className="text-white/40 hover:text-rose-400 ml-1 transition-colors">
                    <span className="material-symbols-outlined text-sm block">close</span>
                  </button>
                )}
              </div>
              <div className="flex gap-2">
                <button onClick={exportPDF} disabled={exporting} className="flex items-center gap-1.5 px-3 py-2 rounded-xl bg-white/5 border border-white/10 text-sm font-semibold text-white/50 hover:bg-white/10 hover:text-white transition-all disabled:opacity-40">
                  <span className="material-symbols-outlined text-[17px]">picture_as_pdf</span> PDF
                </button>
                <button onClick={exportExcel} disabled={exporting} className="flex items-center gap-1.5 px-3 py-2 rounded-xl bg-white/5 border border-white/10 text-sm font-semibold text-white/50 hover:bg-white/10 hover:text-white transition-all disabled:opacity-40">
          ...
          <div className="bg-zinc-900 border border-white/[0.07] rounded-2xl overflow-hidden">
            <table className="w-full text-left">
              ...
              <tbody className="divide-y divide-white/[0.05]">
                {expenses.length === 0 ? (
                  <tr><td colSpan={4} className="px-5 py-10 text-center text-white/30 text-sm">Kayıt bulunamadı.</td></tr>
                ) : expenses.map(exp => (
                  <tr key={exp.id} className="hover:bg-white/[0.03] transition-colors">
                    <td className="px-5 py-3.5">
                      ...
                      {exp.description && <p className="text-xs text-white/40 mt-0.5">{exp.description}</p>}
                    </td>
                    <td className="px-5 py-3.5 text-sm text-white/40 hidden md:table-cell">
                      {new Date(exp.date).toLocaleDateString('tr-TR')}
                    </td>
                    <td className="px-5 py-3.5">
                      <span className={`font-black text-sm ${exp.type === 'income' ? 'text-emerald-400' : 'text-rose-400'}`}>
                      ...
                      <div className="flex items-center justify-end gap-1.5">
                        {exp.invoice_path && (
                          <button onClick={() => setPreviewUrl(`/uploads/${exp.invoice_path}`)}
                            className="p-1.5 rounded-lg bg-white/5 hover:bg-indigo-500/20 hover:text-indigo-300 text-white/40 transition-all" title="Faturayı Gör">
                            <span className="material-symbols-outlined text-base">visibility</span>
                          </button>
                        )}
                        <button onClick={() => handleDelete(exp.id)}
                          className="p-1.5 rounded-lg bg-white/5 hover:bg-rose-500/20 hover:text-rose-300 text-white/40 transition-all" title="Sil">
            ...
            <div className="px-5 py-3 border-t border-white/[0.05] flex items-center justify-between">
              <p className="text-xs text-white/30">Toplam {total} kayıt</p>
              <div className="flex gap-2">
                <button onClick={() => setPage(p => Math.max(1, p - 1))} disabled={page === 1}
                  className="px-3 py-1.5 rounded-lg bg-white/5 border border-white/10 text-xs text-white/60 hover:bg-white/10 disabled:opacity-30 transition-all">Geri</button>
                <button onClick={() => setPage(p => Math.min(totalPages, p + 1))} disabled={page === totalPages}
                  className="px-3 py-1.5 rounded-lg bg-white/5 border border-white/10 text-xs text-white/60 hover:bg-white/10 disabled:opacity-30 transition-all">İleri</button>
              </div>
            </div>
          </div>
        </div>
        ...
        <div className="lg:col-span-4">
          <div className="bg-zinc-900 border border-white/[0.07] p-5 rounded-2xl">
            <h3 className="font-bold text-white text-sm mb-5">Yeni Kayıt Ekle</h3>
            <form onSubmit={handleSubmit} className="space-y-3">
              ...
              >
                <span className="material-symbols-outlined text-white/30 text-2xl">upload_file</span>
                <p className="text-xs text-white/40 mt-1">{file ? <span className="text-indigo-400 font-medium">{file.name}</span> : 'Fatura yükle (opsiyonel)'}</p>
